...
st.set_page_config(
    ...
    menu_items={
        "More info": "https://www.heec.co.uk/resources/extra-funding-allocation-inequality-tool-efit/",            
...
indicators_list = ['Choose an option', 'IMD score (2019)', 'Weighted population', 'Bowel cancer screening coverage', 
    ...
    'Depression prevalence (18+)','Learning disability prevalence (all ages)', 'Dementia prevalence (all ages)', 
...
indicators_list_dict = {
    'IMD score (2019)': selected_icb_for_ind_n_pop['IMD 2019'],
    'Weighted population': selected_icb_for_ind_n_pop['Weighted population'],
    'Bowel cancer screening coverage': selected_icb_for_ind_n_pop['Bowel cancer screening (%)'], 
    'Cervical cancer screening coverage (age 25-49)': selected_icb_for_ind_n_pop['Cervical cancer screening (25-49)(%)'], 
    'Cervical cancer screening coverage (age 50-64)': selected_icb_for_ind_n_pop['Cervical cancer screening (50-64)(%)'], 
    'Asthma prevalence (6+ yrs)': selected_icb_for_ind_n_pop['Asthma: QOF prevalence (6+ yrs)(%)'],
    'Cancer prevalence (all ages)': selected_icb_for_ind_n_pop['Cancer: QOF prevalence (all ages)(%)'],
    'CHD prevalence (all ages)': selected_icb_for_ind_n_pop['CHD: QOF prevalence (all ages)(%)'],
    'MMR vaccine uptake (at least 1 dose - age 1-1.5 yrs)': selected_icb_for_ind_n_pop['MMR vaccine uptake (at least 1 dose - aged 1-1.5 yrs)(%)'],
    'CKD prevalence (18+)': selected_icb_for_ind_n_pop['CKD: QOF prevalence (18+ yrs)(%)'], 
    'COPD prevalence (all ages)': selected_icb_for_ind_n_pop['COPD: QOF prevalence (all ages)(%)'], 
    'Dementia prevalence (all ages)': selected_icb_for_ind_n_pop['Dementia: QOF prevalence (all ages)(%)'], 
    'Depression prevalence (18+)': selected_icb_for_ind_n_pop['Depression: QOF prevalence (18+ yrs)(%)'],
    'Diabetes prevalence (17+)': selected_icb_for_ind_n_pop['Diabetes: QOF prevalence (17+ yrs)(%)'], 
    'Hypertension prevalence (all ages)': selected_icb_for_ind_n_pop['Hypertension: QOF prevalence (all ages)(%)'], 
    'Last BP reading of patients (<80 yrs, with hypertension) <= 140/90 mmHg in the last 12 months': selected_icb_for_ind_n_pop['Last BP reading of patients (<80 yrs, with hypertension), is <= 140/90 mmHg in the last 12 months (%)'],
    'Last BP reading of patients (80+ yrs, with hypertension) <= 150/90 mmHg in the last 12 months': selected_icb_for_ind_n_pop['Last BP reading of patients (80+ yrs, with hypertension), is <= 150/90 mmHg in the last 12 months (%)'],
    'Learning disability prevalence (all ages)': selected_icb_for_ind_n_pop['Learning disability: QOF prevalence (all ages)(%)'], 
    'Mental Health prevalence (all ages)': selected_icb_for_ind_n_pop['Mental Health: QOF prevalence (all ages)(%)'],
    'Obesity prevalence (18+)': selected_icb_for_ind_n_pop['Obesity: QOF prevalence (18+ yrs)(%)'], 
    'Osteoporosis prevalence (50+)': selected_icb_for_ind_n_pop['Osteoporosis: QOF prevalence (50+ yrs)(%)'], 
    'Patients who have a record of BP in the last 5 yrs (45+)': selected_icb_for_ind_n_pop['Patients who have a record of BP in the last 5 yrs (aged 45+ yrs)(%)'], 
    'Patients with diabetes treated with a statin (with a history of CVD - excl. haem. stroke)': selected_icb_for_ind_n_pop['Patients with diabetes treated with a statin (with a history of CVD - excl. haem. stroke)(%)'],
    ...
    'Smoking prevalence (15+)': selected_icb_for_ind_n_pop['Smoking: QOF prevalence (15+ yrs)(%)'],
...
if st.button("**Calculate**", type="primary"):
    ...
    # 2. 'str_extra_output_file': as above but displays also breakdown of total allocation (only if box is checked)
        ...
        str_output_table['Population'] = selected_icb_for_ind_n_pop['Population'].apply(lambda x: "{:,}".format(x))
...
# If weighted pop is selected in any of the three listed indicator boxes, add them in output table, round it to 0 decimal and add comma
                if ind_choice == 'Weighted population':
                    ...
                    str_output_table[round_pop] = str_output_table[round_pop].applymap(
        ...
        for uploaded_weight, name_indicator, matched_uploaded_indicator in zip([uploaded_ind_1_weight, uploaded_ind_2_weight, uploaded_ind_3_weight],
            ...
            if uploaded_weight > 0:
                ...
                str_output_table[round_uploaded] = str_output_table[round_uploaded].applymap(
                    lambda x: "{:,.1f}".format(x) if isinstance(x, (int, float)) else x)
        ...
        str_output_table[round_output] = str_output_table[round_output].applymap(
...
#       st.empty()    
        with st.spinner('Running calculations...'):
                ...
                # add population column and commas
                str_extra_output_table['Population'] = selected_icb_for_ind_n_pop['Population'].apply(lambda x: "{:,}".format(x))
        ...
        # If weighted pop is selected in any of the three listed indicator boxes, add them in output table, round it to 0 decimal and add comma
                        if ind_choice == 'Weighted population':
                            ...
                            round_pop = ['Weighted population']
                            str_extra_output_table[round_pop] = str_extra_output_table[round_pop].applymap(
                                lambda x: "{:,.0f}".format(x) if isinstance(x, (int, float)) else x)
                ...
                if uploaded_ind_1_weight >0:
                    ...
                    str_extra_output_table[round_uploaded] = str_extra_output_table[round_uploaded].applymap(lambda x: "{:,.1f}".format(x) if isinstance(x, (int, float)) else x)
                if uploaded_ind_2_weight >0:
                ...
                if ind_1_weight > 0:
                    ...
                    str_extra_output_table[round_output_x1] = str_extra_output_table[round_output_x1].applymap(lambda x: '{:,.2f}'.format(x) if isinstance(x, (int, float)) else x)
                ...
                if ind_2_weight >0:
                    ...
                    str_extra_output_table[pound_n_round_output_x2] = str_extra_output_table[pound_n_round_output_x2].applymap(lambda x: "{:,.2f}".format(x) if isinstance(x, (int, float)) else x)

                if ind_3_weight >0:
                    str_extra_output_table[f'Allocation by {ind_3_choice} (£)'] = alloc_by_ind_3_weighted   
                    # add commas and round to 2 decimal points the newly calculated columns
                    round_output_x3 = [f'Allocation by {ind_3_choice} (£)']
                    str_extra_output_table[round_output_x3] = str_extra_output_table[round_output_x3].applymap(lambda x: "{:,.2f}".format(x) if isinstance(x, (int, float)) else x)
                ...
                if uploaded_ind_1_weight > 0:
                    ...
                    str_extra_output_table[round_output_u1] = str_extra_output_table[round_output_u1].applymap(lambda x: "{:,.2f}".format(x) if isinstance(x, (int, float)) else x)
                ...
                if uploaded_ind_2_weight > 0:
                    ...
                    str_extra_output_table[round_output_u2] = str_extra_output_table[round_output_u2].applymap(lambda x: "{:,.2f}".format(x) if isinstance(x, (int, float)) else x)
                ...
                if uploaded_ind_3_weight > 0:
                    ...
                    str_extra_output_table[round_output_u3] = str_extra_output_table[round_output_u3].applymap(lambda x: "{:,.2f}".format(x) if isinstance(x, (int, float)) else x)

                if prop_pop >0:
                    str_extra_output_table['Allocation by population size (£)'] = alloc_by_pop
                    # add commas and round to 2 decimal points the newly calculated columns
                    round_output_x_pop = ['Allocation by population size (£)']
                    str_extra_output_table[round_output_x_pop] = str_extra_output_table[round_output_x_pop].applymap(lambda x: "{:,.2f}".format(x) if isinstance(x, (int, float)) else x)
                ...
                str_extra_output_table[round_output_f2] = str_extra_output_table[round_output_f2].applymap(lambda x: "{:,.2f}".format(x) if isinstance(x, (int, float)) else x)
